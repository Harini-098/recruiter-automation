    ...
    while (attempt < maxAttempts) {
      ...
      try {
        ...
        try {
            ...
            if (e.message.includes("Object with guid") || e.message.includes("Target page, context or browser has been closed")) {
                console.log("Playwright connection error detected. Re-initializing browser...");
                await this.close();
                await this.init();
                throw new Error(`Connection error: ${e.message}`); // This will be caught by the while loop and retry
            }
            throw e;
        }
        ...
        if (bodyText.includes("User account is blocked") || bodyText.includes("account has been disabled")) {
            throw new Error("Monster account is blocked");
        }
