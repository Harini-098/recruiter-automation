...
export class DiceService {
    ...
    async updateResume(creds: DiceCredentials): Promise<string> {
        ...
        // --- Handle the "Parse Resume Skills" confirmation dialog ---
        console.log(`[${creds.email}] Checking for "Parse Resume Skills" dialog...`);
        await this.page.waitForTimeout(3000); // Wait for dialog animation

        try {
            // Using a more robust locator for the "Yes" button that handles shadow DOM
            ...
            // Use force click because custom elements with shadow roots can sometimes block standard clicks
            await yesButton.click({ force: true });
            console.log(`[${creds.email}] Clicked "Yes" successfully.`);
        } catch (e) {
            console.log(`[${creds.email}] "Yes" button interaction failed or dialog did not appear: ${e.message}`);
        }
